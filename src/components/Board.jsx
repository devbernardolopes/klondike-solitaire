...
export default function Board() {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const exprs = [
        ...
        'clamp(2px, 0.6vw, 14px)',
        'clamp(4px, 1.4vw, 20px)',
  ...
  const hintByLoc = new Map(); // loc -> { sourceIds: string[], isTarget: boolean, rescueSourceIds: string[], isRescueTarget: boolean }
  for (const h of hints) {
    if (!h || !h.to) continue;
    const t = hintByLoc.get(h.to) || { sourceIds: [], isTarget: false, rescueSourceIds: [], isRescueTarget: false };
    t.isTarget = true;
    if (h.rescue) t.isRescueTarget = true;
    hintByLoc.set(h.to, t);
    if (h.from) {
      const s = hintByLoc.get(h.from) || { sourceIds: [], isTarget: false, rescueSourceIds: [], isRescueTarget: false };
      if (!s.sourceIds.includes(h.cardId)) s.sourceIds.push(h.cardId);
      if (h.rescue && !s.rescueSourceIds.includes(h.cardId)) s.rescueSourceIds.push(h.cardId);
      hintByLoc.set(h.from, s);
    }
  }
  const hintDataFor = (loc) => hintByLoc.get(loc) || { sourceIds: [], isTarget: false, rescueSourceIds: [], isRescueTarget: false };

  const pilesGrid = (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, var(--card-width))', gap: 'clamp(2px, 0.6vw, 14px)', justifyContent: 'center', padding: 'clamp(4px, 1.4vw, 20px)', maxWidth: '100%' }}>
      {handedness === 'right'
        ? [...state.foundations.map((pile, i) => { const h = hintDataFor(`foundation:${i}`); return <Pile key={`f${i}`} loc={`foundation:${i}`} cards={pile} hiddenIds={hiddenIds} onAutoMove={autoMove} moveCard={moveCard} sourceCardIds={h.sourceIds} isHintTarget={h.isTarget} rescueSourceIds={h.rescueSourceIds} isRescueTarget={h.isRescueTarget} animatingIds={animatingIds} won={won} hardBlockBase={hardBlockBase} highlightCard={highlightCard} />; }), <div key="spacer" />, (() => { const h = hintDataFor('waste'); return <Pile key="waste" loc="waste" cards={state.waste} hiddenIds={hiddenIds} onAutoMove={autoMove} moveCard={moveCard} sourceCardIds={h.sourceIds} isHintTarget={h.isTarget} rescueSourceIds={h.rescueSourceIds} isRescueTarget={h.isRescueTarget} animatingIds={animatingIds} won={won} hardBlockBase={hardBlockBase} highlightCard={highlightCard} />; })(), (() => { const h = hintDataFor('stock'); return <Pile key="stock" loc="stock" cards={state.stock} onClick={onStockClick} label={state.stock.length === 0 ? '↻' : ''} hiddenIds={hiddenIds} moveCard={moveCard} sourceCardIds={h.sourceIds} isHintTarget={h.isTarget} rescueSourceIds={h.rescueSourceIds} isRescueTarget={h.isRescueTarget} animatingIds={animatingIds} won={won} hardBlockBase={hardBlockBase} highlightCard={highlightCard} />; })()]
